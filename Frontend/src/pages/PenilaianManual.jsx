...
const PenilaianManual = () => {
    ...
    const onSubmit = (data) => {
        if (data.kode == passwordCluster[cluster - 1]) {
            setLocked(false);
        } else {
            swal(
        ...
        const fetching = await fetch(
            "https://rajabrawijaya.ub.ac.id/api/maba/koreksi/post_data",
            {
                ...
                body: JSON.stringify({
                    nim,
                    jenis_penugasan: "dharma_warta",
                    nilai_tambah: checkBoxRefs.current[index].checked ? 10 : 0,
                }),
            }
        );
        const data = await fetching.json();
    ...
    return (
        <div className="dark:bg-gray-900 max-h-full min-h-screen pt-10 px-20">
            <h6 className="mb-10">Tugas Dharma Abhyaksa</h6>
                        ...
                        <TableBody>
                            {dataMaba.map((data, index) => (
                                ...
                                >
                                    ...
                                    <TableCell>
                                        {data.url_dharma_warta == null ? (
                                            <p>Tidak ada tugas</p>
                                        ) : (
                                            <a
                                                href={data.url_dharma_warta}
                                                target="_blank"
                                                rel="noreferrer"
                                            >
